...
\textbf{针对问题二：} 本文将出海、海返与设施间穿梭需求统一表示为带取送先后关系的人员运输需求，建立异构机型、动态载客量、最多5次海上着陆及途中可选加油约束下的路线集合划分模型。先按起终点聚合需求并构造单类可行架次，再以节约合并产生初始联合运输方案；随后利用合并、转移、交换、二路线重分配和三路线压缩等大邻域算子扩充路线池，并通过整数规划进行全局重组。目标采用严格字典序，依次最小化总飞机使用时间、人员总在途时间和总燃油消耗量，在前三层目标不变时最大化座位利用率。最终为4000名人员安排118个架次，总飞机使用时间为20751分钟，人员总在途时间为248493分钟，总燃油消耗量为161473.10 kg，座位利用率为76.8449
...
\end{figure}
因此最终方案能够完成全部40名应急处置人员、360名增储上产人员和3440名常规倒班人员，并完成154名临时人员，共运输3994人。第一阶段基准和第二阶段最终总飞机使用时间均为32887分钟，即548.12小时。说明几乎所有临时人员都能通过利用已有架次的空余座位和重组联合运输路线加入，无法加入的6位临时人员的临时任务只能为了减少运输开销而取消。最终方案主要指标见表~\ref{tab:q3-results}。
...
在164个架次中，T1、T2和T3分别承担12、90和62个架次，对应飞机使用时间分别为1914、17356和13617分钟。T2承担的架次数最多，说明其16座容量、较低单位距离油耗以及较大的配备数量，使其在有限机队排班中具有较好的综合适应性。规划期内共有21架具体飞机被实际使用。

24架飞机在7个规划日内的排班情况如图~\ref{fig:q3-gantt}所示。彩色区间表示实际架次，浅色延伸部分表示返场后的30分钟周转占用。图中同一飞机的任意两个占用区间均不相交，且全部架次均在当天20:00前返场，直观反映了具体飞机分配、架次不重叠和机场周转约束的可行性。
...
\centering
\includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{outputs/figures/q3_aircraft_gantt.png}
\caption{24架具体飞机的多日排班甘特图}
\label{fig:q3-gantt}

\end{figure}

从每日任务分布看，8月3日至8月9日的总飞机使用时间分别为3739、4244、5054、3946、5417、5318和5169分钟，运输压力主要集中在8月7日至8月9日。A01、A02和A03分别承担66、51和47个架次，对应飞机使用时间为11568、9888和11431分钟，任务并未过度集中于单一机场。
...
最终164个架次中，28个架次仅进行1次海上着陆，31个架次进行2次海上着陆，38个架次进行3次海上着陆，45个架次进行4次海上着陆，另有22个架次使用了允许的5次海上着陆。共有105个架次包含3至5次海上着陆，占全部架次的64.02\%；其中52个架次承担了设施间穿梭人员，说明多站串联以及出海、海返和穿梭联合运输在最终方案中得到充分利用。具体结构见表~\ref{tab:q3-route-structure}。
...
\centering
\caption{不同海上着陆次数的架次结构}
\label{tab:q3-route-structure}
\small
\resizebox{\textwidth}{!}{%
...
\toprule
着陆次数 & 架次数 & 占比 & 平均运输人数 & 平均飞机时间/min & 平均座位利用率 & 穿梭架次数 \\
\midrule
1 & 28 & 17.07\% & 23.39 & 144.79 & 66.58\% & 0 \\
2 & 31 & 18.90\% & 22.06 & 165.42 & 59.51\% & 6 \\
3 & 38 & 23.17\% & 23.34 & 198.84 & 60.56\% & 7 \\
4 & 45 & 27.44\% & 26.82 & 236.69 & 49.26\% & 24 \\
5 & 22 & 13.41\% & 25.50 & 249.91 & 39.51\% & 15 \\
\bottomrule
...
随着海上着陆次数增加，单架次平均运输人数总体提高，但绕行和停靠增加使4至5次着陆架次的平均座位利用率有所下降。这体现了联合运输规模、动态上下客和航段利用效率之间的权衡。海上着陆次数和单架次座位利用率的总体分布如图~\ref{fig:q3-route-structure}所示。
...
最终完成154名临时人员，另有6名临时人员未被安排，临时任务满足率为96.25\%。如图~\ref{fig:q3-temporary}所示，未完成的6名临时人员均为设施间穿梭人员，其时间窗宽度介于1930分钟至6565分钟，并未集中在时间窗最窄的区域。因此，这些人员未被安排的主要原因并不是单人时间窗过紧，而是在 $T\leq32887$ 的严格限制下，其起终点难以与当前非临时架次形成不增加飞机使用时间的联合运输结构。若为其增加独立架次或明显绕行，则会突破第一阶段时间基准。
...
最终输出前，程序没有直接采用主问题内部统计量，而是根据真实人员编号、具体飞机编号和各站实际时刻重新模拟全部架次。独立校验依次检查非临时人员恰好安排一次、临时人员至多安排一次、上下机地点与停靠序号一致、人员在上机后首次到达目的地时下机、动态载客量不超过机型容量、海上着陆次数不超过5次、燃油与安全余油合法、加油设施合法、人员时间窗满足、起飞和返场时刻满足机场运营要求、具体飞机归属正确、相邻架次周转时间不少于30分钟、同一飞机架次不重叠，以及最终总飞机使用时间不超过第一阶段基准。上述检查全部通过。

由于时空路线池由启发式邻域逐步生成，未枚举原问题的全部合法人员组合与运输路线，因此本文不宣称获得原始大规模问题的全局最优解，而将最终结果表述为限定搜索时间和当前生成路线池下，经独立校验得到的最佳已知可行方案。
