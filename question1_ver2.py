"""
Q1: 单向出海运输 — question1_ver2
==================================
目标采用字典序：
1) 最小化总飞机使用时间；
2) 在总飞机使用时间相同时，尽量降低总燃油消耗；
3) 再尽量提高座位利用率；
4) 再尽量降低人员总在途时间。
...
- 到达加油设施时显式比较“加油/不加油”，因此可以处理“当前下一段能飞，但为了后续航段
  必须提前加油”的情况；
- 每个完整路线按 (飞机使用时间, 燃油, -座位利用率, 人员在途时间) 字典序选优。

默认数据路径与 ver1 保持一致，也可通过命令行参数覆盖：
python question1_ver2.py --data-dir docs/reference_formats
"""
...
def lex_better(a: AggregateMetrics, b: Optional[AggregateMetrics]) -> bool:
    """
    a 是否按字典序优于 b：
    time 越小越好 -> fuel 越小越好 -> seat_util 越大越好 -> intransit 越小越好。
    """
    if b is None:
        return True
...
def merge_improvement(a: Sortie, b: Sortie, merged: Sortie) -> Optional[Tuple[int, float, float, int]]:
    """
    若 merged 按字典序优于拆开的 a+b，返回“越大越好”的 improvement tuple；否则 None。
    tuple = (节省时间, 节省燃油, 座位利用率提升, 节省在途时间)
    """
...
def stage2_merge(sorties: Sequence[Sortie]) -> List[Sortie]:
    ...
    - 主目标优先，只有总飞机使用时间相同才比较燃油/座位利用率/在途时间。
    """
    ...
    def push_candidate(i: int, j: int) -> None:
        ...
        if imp is None:
            return
        save_t, save_f, gain_u, save_it = imp
        # heapq 为最小堆，因此全部取负；counter 用于稳定排序。
        heapq.heappush(
            heap,
            (-save_t, -save_f, -gain_u, -save_it, counter, i, j, merged),
        )
        counter += 1
    ...
    merges = 0
    while heap:
        neg_t, neg_f, neg_u, neg_it, _, i, j, merged = heapq.heappop(heap)
