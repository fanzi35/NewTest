...
def bootstrap_merge_batches(
    ...
    for i, batch in enumerate(batches):
        if time.monotonic() >= deadline or len(pool) >= pool.max_patterns:
            break
        ranked = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ops-per-epoch", type=int, default=50)
    parser.add_argument("--neighbors", type=int, default=5)
    parser.add_argument("--bootstrap-neighbors", type=int, default=8)
    parser.add_argument("--bootstrap-triples", type=int, default=300)
    parser.add_argument("--temp-candidates-per-route", type=int, default=4)
    parser.add_argument("--route-variants", type=int, default=6)
    parser.add_argument("--max-patterns", type=int, default=8000)
...
def main() -> None:
    ...
    for epoch in range(1, max(0, args.epochs) + 1):
        if time.monotonic() >= stage1_deadline or len(pool) >= pool.max_patterns:
            break
    ...
    for epoch in range(1, max(0, args.temp_epochs) + 1):
        if time.monotonic() >= total_deadline or len(pool) >= pool.max_patterns:
            break
